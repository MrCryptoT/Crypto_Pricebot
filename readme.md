 ...
 **Variables**
``CryptoSlugstoretrieve`̀ List of all Coinnames to retrieve, Example => 'bitcoin,ethereum,mimblewimblecoin,compound'
``convertto`̀ Name of Coin to convert to (show price in) Example => 'USD'
``extractedpricedata = []`̀ Dont touch - needed to run
``tmp = ''`` Dont touch - needed to run
``slugorname`̀ Set's Displaystyle of Coins, either their 'symbol'  'slug' or 'name'
``seperatorstring`̀ Is what's between the Coins Name and it's price in the template, by Default ' = '
``timebetweentweets`̀ Time between Tweets, by default 14400 in seconds 
